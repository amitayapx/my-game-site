...
function initializeElements() {
  ...
  playerBalanceElement = document.getElementById('player-balance');
...
function updatePlayerRank() {
  ...
  currentRankNameElement.textContent = currentRank.name;
  ...
  playerPointsElement.textContent = currentPlayer.points;
  nextRankPointsElement.textContent = nextRank.points;

  // Set progress bar to 0% since everyone stays at Bronze
  rankProgressElement.style.width = '0%';
}
...
function loadLeaderboards() {
  // Clear existing data
  fortniteWinnersTableElement.innerHTML = '';
  codWinnersTableElement.innerHTML = '';
  topPlayersTableElement.innerHTML = '';
  ...
  // Load Fortnite tournament winners
...
function setupEventListeners() {
  ...
  // Edit buttons
  if (editFortniteWinnersBtn) {
    editFortniteWinnersBtn.addEventListener('click', () => {
      showEditForm('fortnite');
    });
  }

  if (editCodWinnersBtn) {
    editCodWinnersBtn.addEventListener('click', () => {
      showEditForm('cod');
    });
  }

  if (editTopPlayersBtn) {
    editTopPlayersBtn.addEventListener('click', () => {
      showEditForm('top-players');
    });
  }
  
  // Edit rank system button
  const editRankSystemBtn = document.getElementById('edit-rank-system');
  if (editRankSystemBtn) {
    editRankSystemBtn.addEventListener('click', () => {
  ...
  document.getElementById('email-settings-btn').addEventListener('click', () => {
    document.getElementById('emailjs-public-key').value = emailSettings.publicKey;
    document.getElementById('emailjs-service-id').value = emailSettings.serviceId;
    document.getElementById('emailjs-template-id').value = emailSettings.templateId;
    emailSettingsModal.style.display = 'block';
  });

  // Save email settings
  document.getElementById('save-email-settings').addEventListener('click', () => {
    emailSettings.publicKey = document.getElementById('emailjs-public-key').value;
    emailSettings.serviceId = document.getElementById('emailjs-service-id').value;
    emailSettings.templateId = document.getElementById('emailjs-template-id').value;
